...
import com.infosys.msproject.feign.CardFeign;
import com.infosys.msproject.model.Address;
import com.infosys.msproject.model.Card;
import com.infosys.msproject.utility.AddressStateUtil;
import com.infosys.msproject.utility.CardExpiryUtility;
import com.infosys.msproject.validator.CardExpiryValidator;
...
public class CardController {
	...
	public ModelAndView viewCards(Model model,Principal principal,HttpSession session) {
		...
		return new ModelAndView("cards", "cardForm", new Card());
	}
	
	
	@PostMapping(value = "/saveCard")
	public ModelAndView addCard(@Valid @ModelAttribute("cardForm") Card card, BindingResult result,HttpSession session, Model model,Principal principal) {
		
		logger.info("In view save  controller");
		String userId=principal.getName();
		...
		if (result.hasErrors()) {
			...
			modelAndView= new ModelAndView("cards", "cardForm", card);
		} else {
